...
corpus_tokens_millions <- 10          # childLex ≈ 10 млн токенів (Schroeder et al., 2015)
corpus_types_millions  <- nrow(childlex_freqs) / 1000000  # кількість типів у childlex

N<- corpus_tokens_millions + corpus_types_millions
zipf <- round(log10((averaged$mean_freq + 1) / N) + 3, digits = 2)
averaged["zipf"] = zipf

max(zipf, na.rm = TRUE)-min(zipf, na.rm = TRUE)
...
write_xlsx(
  ...
  "results/plots/model_summariesZipf2.xlsx"
)
...
#modelsummary(
#  models,
#  output = "results/model_summariesZipf2.docx",   # або .html, .xlsx, .csv
